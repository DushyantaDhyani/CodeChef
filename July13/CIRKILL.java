...
class Helper{
	...
	private boolean check(int i1,int i2,int i3,int i4){
		...
		if((arr1[i1]==arr1[i2] && arr1[i2]==arr1[i3])|| (arr2[i1]==arr2[i2] && arr2[i2]==arr2[i3])){
			...
			A2=arr2[i2]-arr2[i3];
			B2=arr1[i3]-arr1[i2];
			p=(arr1[i1]+arr1[i2])*1.0/2;
			q=(arr2[i1]+arr2[i2])*1.0/2;
			r=(arr1[i2]+arr1[i3])*1.0/2;
			s=(arr2[i2]+arr2[i3])*1.0/2;
			C1=A1*q-B1*p;
			C2=A2*s-B2*r;
			det=A1*B2-A2*B1;
			if(det==0){
				...
				x=(A2*C1-A1*C2)*1.0/det;
				y=(B2*C1-B1*C2)*1.0/det;
			}
		}
		rad=Math.sqrt(Math.pow((x-arr1[i2]),2)+Math.pow((y-arr2[i2]),2));
		distance=Math.sqrt(Math.pow((x-arr1[i4]),2)+Math.pow((y-arr2[i4]),2));
		if(distance<=rad){
